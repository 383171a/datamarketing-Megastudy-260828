...
-- 3) 생성된 데이터 베이스 안에 테이블 생성하기
CREATE TABLE books (
	book_id INT AUTO_INCREMENT PRIMARY KEY,
    book_title VARCHAR(100),
    author VARCHAR(100),
    publisher VARCHAR(30),
    publish_data VARCHAR(30),
    book_price INT
);

-- 4) 현재 생성된 데이터 베이스 안에 있는 테이블 목록 조회
SHOW TABLES;
...
-- 7) 테이블 안에 값을 저장하는 방법
-- 데이터 시간에는 자동으로 한 번에 복수의 값을 저장하는 방법 배울 예정!!(python) 
INSERT INTO books 
    (book_id, book_title, author, publisher, publish_data, book_price) 
VALUES 
    (1, "박곰희 연금 부자 수업", "박곰희", "인플루엔셜", "2025년 06월", 18900),
    (2, "ETF 투자의 모든것", "문일호", "매일경제신문사", "2025년 08월", 18900),
